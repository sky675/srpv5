local PANEL = {}
	...
	--i need this ok
	local olddraw = PANEL.DrawModel
	function PANEL:DrawModel()
		if(olddraw) then
			olddraw(self) --this doesnt work i guess
		end
		
		self.Entity:DrawModel()
		if(self.modelcache and #self.modelcache != 0) then
			for k,v in pairs(self.modelcache) do
				if(v:IsValid() and !v.nodraw) then v:DrawModel() end
			end
		end
	end
