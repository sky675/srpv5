...
function GM:PlayerSay(client, message)
	...
	message = string.Trim(message) --auto trim the say
	--i dont need this anymore lmao
	--message = hook.Run("ModifyChatMessage", client, message, chatType) or message

	--sorry uglyness and refusal to do this someway else coming through
		--this is better ig
	if(hook.Run("ChatFormatCheck", chatType) == true) then
		if(client:getNetVar("neardeath")) then return end
			
		message = firstToUpper(message)
			
		if(message[-1] == "." or message[-1] == "!" or message[-1] == "?" or message[-1] == "*") then
		else
			message = message.."."
		end 
	end

	if (chatType == "ic" or chatType == "y" or chatType == "w" or chatType == "radio" or chatType == "radiow") then
		...
		if(nut.traits and (nut.traits.hasTrait(client, "big_mute") or nut.traits.hasTrait(client, "big_nonenglish"))) then 
			client:ChatPrint("You are either mute or non-"..nonenglishname..", and as a result cannot speak "..nonenglishname..".")
			return 
		end
...
local IsOp = function(_, client) return client:IsAdmin() or client:IsUserGroup("operator") end
