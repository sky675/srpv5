function GM:LoadNutFonts(font, genericFont)
	...
	surface.CreateFont("nutSmallItalicFont", {
		font = font,
		size = math.max(ScreenScale(6), 17),
		extended = true,
		weight = 500,
	...
	surface.CreateFont("nutSmallItalicBoldFont", {
		font = font,
		size = math.max(ScreenScale(6), 17),
		extended = true,
		weight = 800,
	...
	surface.CreateFont("stalkerBigFont", {
		font = "Dirty Headline",
		size = 36,
		extended = true,
		weight = 500
...
function GM:CharacterListLoaded()
	local hasNotSeenIntro = not nut.config.get("playIntroOnlyOnce", true) or not nut.localData.intro or nil
