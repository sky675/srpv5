...
do
	nut.char.registerVar("name", {
		...
		onAdjust = function(client, data, value, newData)
			...
			if (isstring(name) and override) then
				...
				newData.name = string.gsub(string.Trim(value):sub(1, 70), "[^%w%s-']", "")
			end
		end,
		onPostSetup = function(panel, faction, payload)
			...
			nut.gui.charCreate.nameText = panel
			...
			if (faction) then
				for k, v in SortedPairs(faction.models) do
					...
					icon.DoClick = function(this)
						...
						panel.model:SetModel((type(v) == "string" and v) or v[1], (type(v) == "table" and v[2]))
						if(panel.reloadSkins) then
							panel:reloadSkins(panel)
						end
					end
	...
	nut.char.registerVar("data", {
		...
		noDisplay = true,
		field = "_data",
		onValidate = function() end, --sorry i need this to be here
