...
do
	...
	function playerMeta:isFemale()
		...
		local custom = hook.Run("CustomFemaleModel", model, self)
		...
		function playerMeta:setRestricted(state, noMessage)
			if (state) then
				...
				for k, v in ipairs(self:GetWeapons()) do
					local clip = v:Clip1()
					self.nutRestrictWeps[k] = {class = v:GetClass(), item = ((v.nutItem and v.nutItem:getID()) or nil), ammo = clip}
				end
				...
				if (self.nutRestrictWeps) then
					for k, v in ipairs(self.nutRestrictWeps) do
						local wep = self:Give(v.class, true) --gotta modify this
