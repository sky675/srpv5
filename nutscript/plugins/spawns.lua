...
function PLUGIN:PostPlayerLoadout(client)
	if (self.spawns and table.Count(self.spawns) > 0 and client:getChar()) then
		--[[if(client.setthistime) then return end
		if(!client:getChar():getData("respawn") and !client.setthistime) then
			client:getChar():setData("respawn", true)
			client.setthistime = true
			timer.Simple(1, function()
				client.setthistime = nil
			end)
			return
		end]]
