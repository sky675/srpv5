...
--i changed it where admins can lock/unlock doors now but sure
nut.command.add("doorlock", {
	adminOnly = true,
	syntax = "<bool lockState>",
	desc = "Lock/Unlock a door or button you are looking at. (defaults to lock)",
	onRun = function(client, arguments)
		...
		if (IsValid(entity) and entity:isDoor()) then
			...
			entity:setNetVar("lockState", disabled)
			...
			child:setNetVar("lockState", disabled)
			end)

			-- Tell the player they have made the door (un)disabled.
			client:notifyLocalized("dSet"..(disabled and "" or "Not").."Locked")
