		...
		self.scroll.PaintOver = function(this, w, h)
			...
			if (self.active and IsValid(entry)) then
				...
				if (text:sub(1, 1) == "/") then
					...
					--credit to impulsh/whoever else worked on this for
						--the autocomplete and desc thingys in this
						local currentY = 0
						...
						for k, v in ipairs(self.potentialCommands) do
							...
							if (bSelectedCommand) then
								...
								if (description != "") then
									--might need to wrap
									--local _, height = nut.util.drawText(description, 4, currentY, COLOR_ACTIVE)
									local wrap, height = nut.util.wrapText(description, w*0.8)
									local curhe = currentY
									
					for i = 1, #wrap do
						local info = wrap[i]
						
						local _, he = nut.util.drawText(info, 4, curhe, COLOR_ACTIVE)
						curhe = curhe + he
					end
		
									currentY = curhe + 1--currentY + height + 1
								end
							...
							if (bSelectedCommand and v.syntax) then
								...
								for argument in v.syntax:gmatch("([%[<][%w_]+[%s][%w_]+[%]>])") do
									...
									local width, _ = nut.util.drawText(argument .. "  ", x, currentY, color)
			...
			self.text.OnTextChanged = function(this)
				...
				if (text:sub(1, 1) == "/" and !this.autocompleted) then
					...
					-- offset the index so you don't have to hit tab twice to go past the first command
													--was 1
					if (#self.potentialCommands > 0 and self.potentialCommands[next(self.potentialCommands)].uniqueID == command:sub(2):lower()) then
