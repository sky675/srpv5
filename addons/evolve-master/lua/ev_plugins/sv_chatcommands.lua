...
function PLUGIN:PlayerSay( ply, msg )
	...
	if(string.Left( msg, 1 ) == "@") then
		for _, plugin in ipairs( evolve.plugins ) do
			if(plugin.ChatCommand == "a") then

				local args = string.Split(string.TrimLeft(msg, "@"), " ")

				res, ret = pcall( plugin.Call, plugin, ply, args, args)--args--, string.sub( msg, #command + 3 ), command )
