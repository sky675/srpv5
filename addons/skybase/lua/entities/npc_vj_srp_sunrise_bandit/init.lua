...
ENT.StartHealth = 90--80
ENT.MoveType = MOVETYPE_STEP
ENT.HullType = HULL_HUMAN
ENT.HullSizeNormal = false -- set to false to cancel out the self:SetHullSizeNormal()
---------------------------------------------------------------------------------------------------------------------------------------------
ENT.VJ_NPC_Class = {"CLASS_BANDIT"} 
--putting this up here so its easier
local replacemats = {
	--[origmat] = newmat,
	["models/sky/stalker/sunrise_lone"] = "models/sky/stalker/sunrise_band",

}
--and this too so dont have to modify below this
local maletop = "models/sky/seperate/male_sunrise.mdl"
local femaletop = "models/sky/seperate/female_sunrise.mdl"
...
ENT.SquadName = "bandit" -- Squad name, console error will happen if two groups that are enemy and try to squad!
...
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo,hitgroup,ply) 
	...
	local topmodel = "models/sky/seperate/male_sunrise.mdl"
	if(self.fem or string.find(self.GetModel and self:GetModel() or "", "female")) then
		topmodel = "models/sky/seperate/female_sunrise.mdl"
	end
