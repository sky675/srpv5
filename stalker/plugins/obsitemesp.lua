        ...
        if (client:IsAdmin() and client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle() and NUT_CVAR_ADMINESP:GetBool() and NUT_CVAR_ITEMESP:GetBool()) then
            ...
            for k, v in ipairs(ents.GetAll()) do
				...
				if(v:GetClass() == "nut_money") then
					...
					nut.util.drawText("Money: "..nut.currency.get(v:getAmount()), x, y-size, ColorAlpha(Color(220,220,220,255),alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)	
				end
				--]]
            end
        end
