...
netstream.Hook("sendleaderboard", function(tbl)
    ...
    for k,v in ipairs(tbl) do
        ...
        self.entry[k].rep = self.entry[k]:Add("DLabel")
        self.entry[k].rep:SetText(REP_TRANS[v.rep] .. " Reputation")
        self.entry[k].rep:SetFont("nutSmallFont")
        self.entry[k].rep:SetWide(contW/4)
        self.entry[k].rep:SetContentAlignment(5)
        self.entry[k].rep:SetWrap(true)
        self.entry[k].rep:Dock(LEFT)
        ...
        self.entry[k].rank:SizeToContents()
        self.entry[k].rank:SetWrap(true)
        self.entry[k].rank:Dock(LEFT)

        self.entry[k].xp = self.entry[k]:Add("DLabel")
        self.entry[k].xp:SetText(v.xp)
        self.entry[k].xp:SetFont("nutSmallFont")
        self.entry[k].xp:SetTextColor(Color(40,115,177))
        self.entry[k].xp:SetContentAlignment(5)
        self.entry[k].xp:SizeToContents()
        self.entry[k].xp:SetWrap(true)
