...
function PLUGIN:PlayerDeath(victim, inflictor, attacker)
	...
	if victim:getChar() then
		...
		nut.inventory.instance("grid", {w = victimInv:getWidth(), h = victimInv:getHeight()}, function(inventory)
			...
			--get all money atm (getData("atm")) 
			--+ inv (getMoney(true)) 
			--+ stash (GetStash, load like below, and check instances)
			--[[local stash = char:GetStash()

			--get like some % of all money (?)
			local totake = 10000
			local donetake = false

			--take money out of atm
			if(char:getData("atm", 0) >= totake) then --if they meet or have more than take, take it from atm and be done
				char:setData("atm", char:getData("atm", 0)-totake)
				donetake = true
			else
				local atm = char:getData("atm", 0)
				totake = totake-atm
				char:setData("atm", 0) --reset
			end

			--now inv
			if(!donetake) then
				local left = char:takeMoney(totake, true)
				if(left == true) then
					donetake = true
				end
			end
			...
			if(!donetake) then
				...
				for k,v in pairs(stash) do
					...
					if(item and item:getData("money")) then
						local mone = item:getData("money")
						if(mone >= totake) then  --this is enough
							item:setData("money", mone-totake) --hopefully this works
							break
						else
							totake = totake-mone
							item:setData("money", 0)
						end
					end
				end
			end

			char:setData("moneydeath") --reset it

			victim:notify("You have used your money death status! Thank you for your 10k, deducted from your overall balance. Please buy another for your next death!")
			]]
			--if there isnt enough, dont worry about it, itll go negative and they cant withdraw until its not negative
