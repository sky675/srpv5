...
local matreplace = {	
	["eco_orang"] = "models/sky/stalker/eco_green",
}
...
ITEM:postHook("Equip", function(item, result, data)
	...
	for k,v in ipairs(mats) do
		if(string.find(v, "eco_orang")) then
			mat = k-1
		end
	end
	if(mat) then
		item.player:SetSubMaterial(mat, "models/sky/stalker/eco_green")
		local sub = item.player:getChar():getData("submat", {})
		sub["eco_orang"] = "models/sky/stalker/eco_green"
