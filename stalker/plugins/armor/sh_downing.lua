...
nut.command.add("devutildura", {
	...
	onRun = function(client, arguments)
		...
		for k, v in pairs(items) do
			str = str..";"..v.name.." - "..tostring(v:getData("durability", "unset (max)")).." - equip:"..tostring(v:getData("equip"))
		end
...
if(SERVER) then
	...
	hook.Add("EntityTakeDamage", "disablegrendamage", function(target, dmg)
		...
		if(target:IsPlayer() and IsValid(dmg:GetAttacker()) and (dmg:GetAttacker():GetClass() == "func_door" or dmg:GetAttacker():GetClass() == "obj_vj_flareround")) then
			return true
		end
