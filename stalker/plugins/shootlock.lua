...
function PLUGIN:EntityTakeDamage(entity, dmgInfo)
	if (entity:GetClass() == "prop_door_rotating" and (entity.nutNextBreach or 0) < CurTime()) then
		...
		if (handle and dmgInfo:IsBulletDamage() and dmgInfo:GetDamage() >= 30) then
			local client = dmgInfo:GetAttacker()
			local position = dmgInfo:GetDamagePosition()

			if(!client:IsPlayer()) then return end --had npcs hit the door and it triggered this
			...
			if (IsValid(client)) then
				...
				if (IsValid(weapon) and weapon:GetClass() == "weapon_shotgun") then
					...
					entity:BlastDoor(client:GetAimVector() * 380, 180)
			...
			if (IsValid(client) and position:Distance(entity:GetBonePosition(handle)) <= 12) then
				...
				if(entity:getNetVar("lockState")) then
...
hook.Add("CanPlayerBustLock", "disableNoOpen", function(client, entity)
	if(entity:getNetVar("disabled") or entity.lock) then
		return false
	end
end)